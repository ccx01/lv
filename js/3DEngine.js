...
DisplayObject3D.prototype.init = function (container){
	...
	var li = container.find("li").size();
		for (i=li; i < this.pointsArray.length; i++){
			this.container.append('<li><img src="images/1.jpg"/></li>');
		}
};	
...
Scene3D.prototype.renderCamera = function (camera){
	...
	for(var i = 0 ; i< this.sceneItems.length; i++){
		...
		var screenPoints = this.Transform3DPointsTo2DPoints(obj.pointsArray, axisRotation, camera);
			...
			$("#"+obj.containerId).find("li").each(function(k){
				$(this).css({
					'position':'absolute',
					'top': screenPoints[k].x + 'px',
					'left': screenPoints[k].y + 'px',
					'opacity':(screenPoints[k].scaleFactor-1),
					'z-index':Math.floor(screenPoints[k].scaleFactor)
				});
			});
